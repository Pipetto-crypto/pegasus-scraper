		...
		do
			...
			then 
				...
				then
					...
					sed -i "0,/^$/{s|^$|$PS2|}" $f/metadata.pegasus.txt > /dev/null
				...
				then
					...
					sed -i "0,/^$/{s|^$|$GC|}" $f/metadata.pegasus.txt > /dev/null			
				elif awk 'NR==2' $f/metadata.pegasus.txt|grep 'shortname: wii' > /dev/null;
				then
					echo -e "\nAdding Dolphin MMJR launch command"; sleep 3
					WII='launch: am start\n -n org.mm.jr/org.dolphinemu.dolphinemu.ui.main.MainActivity\n -a android.intent.action.VIEW\n --es AutoStartFile "{file.path}"\n'
					sed -i "0,/^$/{s|^$|$WII|}" $f/metadata.pegasus.txt > /dev/null	
				...
				then
					...
					sed -i "0,/^$/{s|^$|$PSX|}" $f/metadata.pegasus.txt > /dev/null
				...
				then
					...
					sed -i "0,/^$/{s|^$|$N64|}" $f/metadata.pegasus.txt > /dev/null
				...
				then
					...
					sed -i "0,/^$/{s|^$|$NDS|}" $f/metadata.pegasus.txt > /dev/null
				fi
			fi
		done
		break
