	...
	then
		...
		do
			...
			then 
				...
				elif awk 'NR==2' $f/metadata.pegasus.txt|grep 'shortname: gc' > /dev/null;
				then
				...
				elif awk 'NR==2' $f/metadata.pegasus.txt|grep 'shortname: wii' > /dev/null;
				then
					echo -e "\nAdding Dolphin MMJR launch command"; sleep 3
					WII='launch: am start\n -n org.mm.jr/org.dolphinemu.dolphinemu.ui.main.MainActivity\n -a android.intent.action.VIEW\n --es AutoStartFile "{file.path}"\n'
					sed -i "0,/^$/{s|^$|$WII|}" $f/metadata.pegasus.txt	
				elif awk 'NR==2' $f/metadata.pegasus.txt|grep 'shortname: psx' > /dev/null;
				then
				...
				elif awk 'NR==2' $f/metadata.pegasus.txt|grep 'shortname: n64' > /dev/null;
				then
					echo -e "\nAdding M64Plus Fz launch command"; sleep 3
				...
				elif awk 'NR==2' $f/metadata.pegasus.txt|grep 'shortname: nds' > /dev/null;
				then
